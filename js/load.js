$(document).ready(function(){
   ...
   // App.call("getAD",function(res1){
    ...
    $.ajax({
        // url:"/mobile/report/life/summaryAnnualWork.do",
        url:"http://localhost:63342/summaryAnnualWork/js/seat2.json",
        data: {"um": UM_NUM,"method":"summary"},
        type: 'GET',
        dataType: 'JSON',
        success:function(n){
        	if(n.charityActvData.length==0){
        		$(".swiper-wrapper").hide();
            	$("<div class='addPage font18pt'>抱歉，您不符合角色要求无法浏览！</div>").appendTo("#container");
        	}else{
        		var data=n.charityActvData[0];
                $(".registTime").text(fomatDate(data.REGIST_TIME)); // 报名参与时间
                $(".charityDays").attr('data-to',data.CHARITY_DAYS); // 公益天数：用月末最后一天- 报名参与时间  算出参加公益天数 
                $(".totalStepNum").attr('data-to',data.TOTAL_STEP_NUM); // 累计上传步数
                $(".totalBookNum").attr('data-to',data.TOTAL_BOOK_NUM); // 累计捐书数
                $(".totalCustNum").attr('data-to',data.TOTAL_CUST_NUM); // 累计客户数：爱心人数
                $(".monthAddCustNum").attr('data-to',data.MONTH_ADD_CUST_NUM); // 当月新增客户数：当月爱心接力人数
                $(".monthActvCustNum").attr('data-to',data.MONTH_ACTV_CUST_NUM);// 当月活跃客户数 ：本月健行公益人数  当月上传次数>0  的客户数
